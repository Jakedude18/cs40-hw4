...
CVFFields_T CVFCompressor(rgbBlock_T rgbBlock){
        ...
        CVFFields->PB = pbSum / PIXSINBLOCK;
        CVFFields->PR = prSum / PIXSINBLOCK;
...
rgbBlock_T CVFDecompressor(CVFFields_T CVFFields){
        ...
        for(int i = 0; i < PIXSINBLOCK; i++){
                ...
                ((Pnm_rgb)UArray_at(rgbBlock->block,i))->red = 1.0 * y + 0.0 * pb + 1.402 * pr;
                ((Pnm_rgb)UArray_at(rgbBlock->block,i))->green = 1.0 * y - 0.344136 * pb - 0.714136 * pr;
                ((Pnm_rgb)UArray_at(rgbBlock->block,i))->blue = 1.0 * y + 1.772 * pb + 0.0 * pr;
        }
